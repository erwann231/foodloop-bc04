...
function CheckoutForm({ orderId, amount }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (paymentIntent.status === 'succeeded') {
                ...
                if (res.ok) router.push(`/orders/${orderId}?paid=true`);
                else setError('Paiement confirmé mais erreur lors de la mise à jour.');
            }
        } catch (err) {
    ...
    return (
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            <PaymentElement />
            {error && (
                <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '8px', padding: '0.75rem 1rem', color: '#991b1b', fontSize: '0.9rem' }}>
                    {error}
                </div>
            )}
...
function PaymentContent() {
    ...
    if (loading) return <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--color-muted)' }}>Initialisation du paiement...</div>;
    ...
    if (error) return (
        <div style={{ maxWidth: '500px', margin: '4rem auto', textAlign: 'center', padding: '1rem' }}>
            ...
            <Link href="/orders" style={{ background: 'var(--color-primary)', color: '#fff', padding: '0.75rem 2rem', borderRadius: '8px', fontWeight: 700, textDecoration: 'none' }}>
                Mes commandes
            </Link>
        </div>
    );
        ...
        <div style={{ maxWidth: '500px', margin: '0 auto', padding: '2rem 1rem' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', padding: '2rem', boxShadow: 'var(--shadow)' }}>
                <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
                    ...
                    <h1 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--color-primary)', margin: 0 }}>Paiement</h1>
                    <p style={{ color: 'var(--color-muted)', marginTop: '0.25rem', fontSize: '0.9rem' }}>
                        Commande #{orderId?.slice(0, 8).toUpperCase()}
                    </p>
                </div>
                {clientSecret && (
                    <Elements stripe={stripePromise} options={{ clientSecret, appearance: { theme: 'stripe', variables: { colorPrimary: '#2D6A4F', borderRadius: '8px' } } }}>
...
export default function PaymentPage() {
    return (
        <Suspense fallback={<div style={{ textAlign: 'center', padding: '4rem', color: 'var(--color-muted)' }}>Chargement...</div>}>
            <PaymentContent />
        </Suspense>
    );
}